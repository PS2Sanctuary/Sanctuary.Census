...
namespace Sanctuary.Census.Common.Util
{
    public ref partial struct SpanReader<T> where T : unmanaged, IEquatable<T>
    {
        ...
        public bool TryReadTo(out ReadOnlySpan<T> span, T delimiter, bool advancePastDelimiter = true)
        {
            ...
            if (!TryAdvanceTo(delimiter, advancePastDelimiter))
            {
                span = default;
                return false;
            }
        ...
        /// </summary>
        /// <param name="delimiters">The delimiters to search for.</param>
        /// <param name="advancePastDelimiter"><c>True></c> to move past the <paramref name="delimiters"/>, if found.</param>
        /// <returns><c>True</c> if the given <paramref name="delimiters"/> was found, otherwise <c>False</c></returns>
        public bool TryAdvanceToAny(ReadOnlySpan<T> delimiters, bool advancePastDelimiter = true)
        {
            if (End)
                return false;

            int index = Span[Consumed..].IndexOfAny(delimiters);
            if (index == -1)
                return false;
